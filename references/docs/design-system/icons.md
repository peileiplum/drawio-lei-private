...
Preferred AWS rendering uses the official AWS Architecture Icons color package from `https://aws.amazon.com/architecture/icons/`. When the package is available at `AWS_ARCHITECTURE_ICONS_DIR`, `~/.drawio-lei/aws-architecture-icons`, or `/tmp/aws-architecture-icons`, common `aws.*` references are embedded as official SVG image cells. Export those diagrams with draw.io Desktop for faithful output.

If the official package is unavailable, icons fall back to the `mxgraph.aws4.*` shape prefix.

#### Compute

| Service | Icon Reference | Shape |
|---------|---------------|-------|
| Lambda | `aws.lambda` | official SVG, fallback `mxgraph.aws4.lambda` |
| EC2 | `aws.ec2` | official SVG, fallback `mxgraph.aws4.ec2_instance` |
...
| Service | Icon Reference | Shape |
|---------|---------------|-------|
| S3 | `aws.s3` | official SVG, fallback `mxgraph.aws4.s3` |
| DynamoDB | `aws.dynamodb` | official SVG, fallback `mxgraph.aws4.dynamodb` |
| RDS | `aws.rds` | official SVG, fallback `mxgraph.aws4.rds_instance` |
...
| Service | Icon Reference | Shape |
|---------|---------------|-------|
| API Gateway | `aws.api-gateway` | official SVG, fallback `mxgraph.aws4.api_gateway` |
| CloudFront | `aws.cloudfront` | official SVG, fallback `mxgraph.aws4.cloudfront` |
| Route 53 | `aws.route53` | official SVG, fallback `mxgraph.aws4.route_53` |
| VPC | `aws.vpc` | official SVG, fallback `mxgraph.aws4.vpc` |
| ALB | `aws.alb` | official SVG, fallback `mxgraph.aws4.application_load_balancer` |
