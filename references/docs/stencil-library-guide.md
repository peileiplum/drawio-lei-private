...
1. official AWS Architecture Icons package for current AWS color assets
2. known icon mappings in the design system
3. semantic shapes when exact icon resolution cannot be guaranteed
...
6. For AWS diagrams, prefer the official AWS Architecture Icons color package from `https://aws.amazon.com/architecture/icons/` when available. Embed SVG assets as `shape=image` cells and export with draw.io Desktop for faithful rendering; use `mxgraph.aws4.*` only as a fallback when the official package is unavailable.
