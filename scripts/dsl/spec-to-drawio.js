...
const ICON_ALIASES = {
  ...
  'aws.route53': 'aws.route_53',
  'aws.step-functions': 'aws.step_functions',
  'aws.transit-gateway': 'aws.transit_gateway',
  'aws.direct-connect': 'aws.direct_connect',
...
const AWS_OFFICIAL_ICON_ROOTS = [
  process.env.AWS_ARCHITECTURE_ICONS_DIR,
  process.env.HOME ? resolve(process.env.HOME, '.drawio-lei/aws-architecture-icons') : null,
  '/tmp/aws-architecture-icons'
].filter(Boolean)

const AWS_OFFICIAL_ICON_PATHS = {
  'aws.lambda': 'Architecture-Service-Icons_07312025/Arch_Compute/64/Arch_AWS-Lambda_64.svg',
  'aws.ec2_instance': 'Architecture-Service-Icons_07312025/Arch_Compute/64/Arch_Amazon-EC2_64.svg',
  'aws.s3': 'Architecture-Service-Icons_07312025/Arch_Storage/64/Arch_Amazon-Simple-Storage-Service_64.svg',
  'aws.rds_instance': 'Architecture-Service-Icons_07312025/Arch_Database/64/Arch_Amazon-RDS_64.svg',
  'aws.dynamodb': 'Architecture-Service-Icons_07312025/Arch_Database/64/Arch_Amazon-DynamoDB_64.svg',
  'aws.cloudfront': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_Amazon-CloudFront_64.svg',
  'aws.route_53': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_Amazon-Route-53_64.svg',
  'aws.api_gateway': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_Amazon-API-Gateway_64.svg',
  'aws.application_load_balancer': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_Elastic-Load-Balancing_64.svg',
  'aws.elb': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_Elastic-Load-Balancing_64.svg',
  'aws.vpc': 'Resource-Icons_07312025/Res_Networking-Content-Delivery/Res_Amazon-VPC_Virtual-private-cloud-VPC_48.svg',
  'aws.internet_gateway': 'Resource-Icons_07312025/Res_Networking-Content-Delivery/Res_Amazon-VPC_Internet-Gateway_48.svg',
  'aws.nat_gateway': 'Resource-Icons_07312025/Res_Networking-Content-Delivery/Res_Amazon-VPC_NAT-Gateway_48.svg',
  'aws.direct_connect': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_AWS-Direct-Connect_64.svg',
  'aws.transit_gateway': 'Architecture-Service-Icons_07312025/Arch_Networking-Content-Delivery/64/Arch_AWS-Transit-Gateway_64.svg'
}
...
function normalizeIconName(icon) {
  return icon ? ICON_ALIASES[icon] || icon : null
}
...
export function resolveOfficialAwsIconImage(icon) {
  if (!icon || process.env.DRAWIO_LEI_DISABLE_OFFICIAL_AWS_ICONS === '1') return null

  const normalizedIcon = normalizeIconName(icon)
  const relativePath = AWS_OFFICIAL_ICON_PATHS[normalizedIcon]
  if (!relativePath) return null

  for (const root of AWS_OFFICIAL_ICON_ROOTS) {
    const fullPath = resolve(root, relativePath)
    if (existsSync(fullPath)) {
      return encodeSvgDataUri(readFileSync(fullPath, 'utf-8'))
    }
  }
...
export function generateNodeStyle(node, theme) {
  ...
  const iconRef = node.icon || deriveNodeIcon(node)
  const officialAwsImage = resolveOfficialAwsIconImage(iconRef)
  const iconShape = officialAwsImage ? 'image' : resolveIconShape(iconRef)
  let effectiveShapeStyle = shapeStyle
  const parts = []
  if (officialAwsImage) {
    effectiveShapeStyle = 'shape=image'
    parts.push(effectiveShapeStyle)
    parts.push('html=1')
    parts.push('whiteSpace=wrap')
    parts.push('imageAspect=0')
    parts.push('aspect=fixed')
    parts.push(`image=${officialAwsImage}`)
    parts.push('fillColor=none')
    parts.push('strokeColor=none')
    parts.push('strokeWidth=0')
...
export function validateXml(xml) {
  ...
  if (!/<mxCell\b(?=[^>]*\bid=["']0["'])[^>]*\/?>/i.test(xml)) {
    errors.push('Missing required <mxCell id="0"/>')
  }
  if (!/<mxCell\b(?=[^>]*\bid=["']1["'])(?=[^>]*\bparent=["']0["'])[^>]*\/?>/i.test(xml)) {
