...
describe('Phase 2.4: generateNodeStyle with icon', () => {
  ...
  it('node with icon aws.lambda should use official AWS image when available, otherwise aws4 fallback', () => {
    const style = generateNodeStyle({ id: 'n1', label: 'Lambda', icon: 'aws.lambda' }, theme)
    if (resolveOfficialAwsIconImage('aws.lambda')) {
      assert.ok(style.includes('shape=image'), 'should contain shape=image')
      assert.ok(style.includes('data:image/svg+xml'), 'should embed official AWS SVG data')
      assert.ok(style.includes('fillColor=none'), 'should not recolor official AWS SVG')
    } else {
...
describe('validateXml', () => {
  ...
  it('should accept draw.io root cells with whitespace before self-closing slash', () => {
    const xml =
      '<mxGraphModel>' +
      '<root>' +
      '<mxCell id="0" />' +
