...
/**
 * Extract a selected page's raw mxGraphModel XML from a .drawio file.
 * This is useful for preview/export paths that must preserve draw.io's
 * original geometry and style instead of round-tripping through the DSL.
 * @param {string} drawioFileText
 * @param {{ page?: string|number }} options
 * @returns {string}
 */
export function extractMxGraphModelFromDrawio(drawioFileText, options = {}) {
  if (typeof drawioFileText !== 'string' || drawioFileText.trim() === '') {
    throw new Error('extractMxGraphModelFromDrawio: input must be a non-empty string')
  }
  ...
  const { mxGraphModelXml } = parseMxGraphModelXml(decoded)
